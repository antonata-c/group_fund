...
class CollectViewSetTest(APITestCase):
    ...
    def test_create_collect(self):
        ...
        response = self.client.post(self.list_url, self.collect_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Collect.objects.count(), objects_count + 1)
        self.assertEqual(Collect.objects.last().title, self.collect_data["title"])
...
class PaymentViewSetTest(APITestCase):
    ...
    def test_create_payment(self):
        ...
        response = self.client.post(self.list_url, self.payment_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Payment.objects.count(), objects_count + 1)
        self.assertEqual(Payment.objects.last().amount, self.payment_data["amount"])
